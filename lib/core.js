...
module.exports = class Core extends Emitter{
	...
	this.on("responding", ctx => {
		...
		self.responsemw.callback( ctx => self.emit("response",ctx) )(ctx);
	    }else{
	...
	this.on("parsed", ctx => {
		...
		self.parsedmw.callback( ctx => self.emit("process",ctx) )(ctx);
	    }else{
		throw new Error("context status error!");
	    }
	});
    ...
    enqueue(opt){
	let self = this;
	this._scheduler.key(opt.limiter||"default").submit( done => {
	    let ctx = self._createContext(opt);
	    ctx.done = done;
	    if(ctx.next().value === ctx.REQ){
		self.requestmw.callback( ctx => self.emit("request",ctx) )(ctx);
	    }else{
		throw new Error("context status error!");
	    }
	});
